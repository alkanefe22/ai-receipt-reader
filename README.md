...
- One extractor fails → the arbiter model reads the whole document in its place (the UI notes this). The comparison is then between two readers with no third vote left.
...
Without API keys (or with `DEMO_MODE=true`), the app replays **recorded model responses** for five bundled sample documents. The responses still go through the real consensus, arbitration and validation code, so the demo shows the actual pipeline at zero cost. Uploads are disabled in this mode.
